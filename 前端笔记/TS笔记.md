...
如果已经看完 [工程化的起步准备](https://vue3.chengpeiquan.com/guide.html) 一章，相信此时的已经对 Node 工程项目有了足够的认识了，在此之前的所有代码都是使用 JavaScript 编写的，接下来这一节，将开始介绍 TypeScript ，这是一门新的语言，但是上手非常简单。
...
![TypeScript 的流行程度（来自 GitHub 年度统计报告）](https://vue3.chengpeiquan.com/assets/img/github-top-languages-dark.jpg)

TypeScript 的流行程度（来自 GitHub 年度统计报告）
...
将继续使用 [Hello Node](https://vue3.chengpeiquan.com/guide.html#hello-node) 这个 demo ，或者可以再建一个新 demo ，依然是在 `src` 文件夹下，创建一个 `ts` 文件夹归类本次的测试文件，然后创建一个 `index.ts` 文件在 `ts` 文件夹下。

> [!tip] TypeScript 语言对应的文件扩展名是 `.ts` 。
...
> [!tip] 请注意， `dev:ts` 这个 script 是用了 `ts-node` 来代替原来在用的 `node` ，因为使用 `node` 无法识别 TypeScript 语言。

把 [为什么需要类型系统](https://vue3.chengpeiquan.com/typescript.html#%E4%B8%BA%E4%BB%80%E4%B9%88%E9%9C%80%E8%A6%81%E7%B1%BB%E5%9E%8B%E7%B3%BB%E7%BB%9F) 里面提到的例子放到 `src/ts/index.ts` 里：
...
在 [Hello TypeScript](https://vue3.chengpeiquan.com/typescript.html#hello-typescript) 的体验中，相信能够感受到 TypeScript 编程带来的好处了，代码的健壮性得到了大大的提升！

并且应该也能够深刻理解，TypeScript 的类型系统并不会给编程带来极高的门槛或者说开发障碍，这是以非常小的成本换取巨大收益的举措。

> [!tip] 如果还没有体验这个 demo ，建议先按教程跑一下，然后来讲解不同的 JavaScript 类型应该如何在 TypeScript 里定义，接下来的时间里，可以一边看，一边在 demo 里实践。
...
但是！如果一开始是 `[]` ，那么就必须显式的指定数组类型（取决于当前项目的 [tsconfig.json](https://vue3.chengpeiquan.com/typescript.html#%E4%BA%86%E8%A7%A3-tsconfig-json) 配置，可能会引起报错）：
...
> [!tip] 对象的类型定义通常采用 Upper Camel Case 大驼峰命名法，也就是每个单词的首字母大写，例如 `UserItem` 、 `GameDetail` ，这是为了跟普通变量进行区分（变量通常使用 Lower Camel Case 小驼峰写法，也就是第一个单词的首字母小写，其他首字母大写，例如 `userItem` ）。
...
如果类上面本身有方法存在，接口在继承的时候也要相应的实现，当然也可以借助在 [对象（接口）](https://vue3.chengpeiquan.com/typescript.html#%E5%AF%B9%E8%B1%A1-%E6%8E%A5%E5%8F%A3) 提到的 `Omit` 帮助类型来去掉这些方法。
...
所以这里先插入一个知识点，在介绍 [对象（接口）](https://vue3.chengpeiquan.com/typescript.html#%E5%AF%B9%E8%B1%A1-%E6%8E%A5%E5%8F%A3) 和 [类](https://vue3.chengpeiquan.com/typescript.html#%E7%B1%BB) 的类型定义时，提到 `Omit` 的帮助类型，它的类型里面有一个写法是 `string | number | symbol` ，这其实是 TypeScript 的一个联合类型。
...
>[!tip] 注意在上面 `counter` 函数的 `console.log` 语句里，使用了一个 ` `` ` 符号来定义字符串，这是 ES6 语法里的 [模板字符串](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Template_literals) ，它和传统的单引号 / 双引号相比更为灵活，特别是遇到字符串需要配合多变量拼接和换行的情况。
...
最后这个使用场景在 Vue 单组件的 [DOM 元素与子组件](https://vue3.chengpeiquan.com/component.html#dom-%E5%85%83%E7%B4%A0%E4%B8%8E%E5%AD%90%E7%BB%84%E4%BB%B6) 一节里也有相关的讲解。

当决定使用联合类型的时候，大部分情况下可能需要对变量做一些类型判断再写逻辑，当然有时候也可以无所谓，就像第一个例子拼接字符串那样。

这一小节在这里做简单了解即可，因为下面会继续配合不同的知识点把这个联合类型再次拿出来讲，比如 [函数的重载](https://vue3.chengpeiquan.com/typescript.html#%E5%87%BD%E6%95%B0%E7%9A%84%E9%87%8D%E8%BD%BD) 部分。
...
实际业务中会遇到有一些函数入参是可选，可以用和 [对象（接口）](https://vue3.chengpeiquan.com/typescript.html#%E5%AF%B9%E8%B1%A1-%E6%8E%A5%E5%8F%A3) 一样，用 `?` 来定义：
...
在未来的实际开发中，可能会接触到一个 API 有多个 TS 类型的情况，比如 Vue 的 [watch API](https://vue3.chengpeiquan.com/component.html#api-%E7%9A%84-ts-%E7%B1%BB%E5%9E%8B) 。
...
> [!tip] 注意这里的入参和返回值使用了 TypeScript 的 [联合类型](https://vue3.chengpeiquan.com/typescript.html#%E8%81%94%E5%90%88%E7%B1%BB%E5%9E%8B) ，不了解的话请先重温知识点。
...
如果要强制确认类型，需要使用 TS 的 [类型断言](https://vue3.chengpeiquan.com/typescript.html#%E7%B1%BB%E5%9E%8B%E6%96%AD%E8%A8%80) （留意后面的 `as` 关键字）：
...
还记得在 [为什么需要类型系统](https://vue3.chengpeiquan.com/typescript.html#%E4%B8%BA%E4%BB%80%E4%B9%88%E9%9C%80%E8%A6%81%E7%B1%BB%E5%9E%8B%E7%B3%BB%E7%BB%9F) 的用的那个例子吗？再次放到 `src/ts/index.ts` 里：
...
> [!tip] 使用 any 的目的是让在开发的过程中，可以不必在无法确认类型的地方消耗太多时间，不代表不需要注意代码的健壮性。
...
在 TS 文件里导入并使用这个包的时候，会编译失败，比如在前面的 [Hello TypeScript](https://vue3.chengpeiquan.com/typescript.html#hello-typescript) demo 里敲入以下代码：
...
在讲解 [函数的重载](https://vue3.chengpeiquan.com/typescript.html#%E5%87%BD%E6%95%B0%E7%9A%84%E9%87%8D%E8%BD%BD) 的时候，提到了一个用法：
...
当一个变量应用了 [联合类型](https://vue3.chengpeiquan.com/typescript.html#%E8%81%94%E5%90%88%E7%B1%BB%E5%9E%8B) 时，在某些时候如果不显式的指明其中的一种类型，可能会导致后续的代码运行报错。
...
> [!tip] 使用类型断言可以让 TypeScript 不再检查该代码，默认是正确无误的。

所以，请务必保证这段代码真的是正确的！

### 类型推论

还记得在讲 [原始数据类型](https://vue3.chengpeiquan.com/typescript.html#%E5%8E%9F%E5%A7%8B%E6%95%B0%E6%8D%AE%E7%B1%BB%E5%9E%8B) 的时候，最后提到的：
...
> [!tip] 笔者 2024-04-06 注：在 build script 中新增了一个选项 `--skipLibCheck` ，关于这个改动请见 [干净的 TypeScript 项目在编译时报错 Cannot find module 'undici-types' 的原因和解决](https://chengpeiquan.com/article/typescript-error-cannot-find-module-undici-types.html) 一文。
...
> [!tip] 通常还需要配置一个 `--module` 选项，用于决定编译后是 CJS 规范还是 ESM 规范，但如果缺省，会根据 `--target` 来决定。
